...
bool ListView::init()
{
	...
	m_slider = CCSprite::create("common/btn_4.png");
	m_sliderBg = CCSprite::create("common/btn_4.png");
	/*m_sliderBg = CCSprite::create("fortune/slider_2-1.png");
	m_slider = CCSprite::create("fortune/slider_2-3.png");
	addChild(m_sliderBg);
	float scale = 1.0f;
	...
	auto slider1 = CCSprite::create("fortune/slider_2-2.png");
	...
	auto slider2 = CCSprite::create("fortune/slider_2-2.png");
...
//void ListView::onSliderMoved(cocos2d::CCPoint offset)
//{
//	float viewHeight = m_size.height;
//	float containerHeight = m_container->getContentSize().height;
//	float maxHeight = m_size.height - m_slider->getContentSize().height * 0.5f;
//	float minHeight = m_slider->getContentSize().height * 0.5f;
//	
//	float containerPosY = m_container->getPosition().y;
//	float containerOffsetHeight = (containerHeight - viewHeight) / (maxHeight - minHeight) * offset.y;
//	m_container->setPositionY(containerPosY - containerOffsetHeight);
//	dragback();
//}
...
void ListView::ccTouchMoved(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
	...
	if (m_isSliding)
	{
		...
		if (newY < m_slider->getContentSize().height * 2 * m_slider->getScale()-8)
		{
			//newY = m_sliderBg->getPositionY() + m_sliderBg->getContentSize().height / (2 * m_sliderBg->getScale()) - m_slider->getContentSize().height*1.1f / m_sliderBg->getScale();
			newY = m_slider->getContentSize().height * 2 * m_slider->getScale()-8;
		}
